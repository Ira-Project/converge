...
import { type ActivityType, KnowledgeZapQuestionType, Roles } from "@/lib/constants";
import { Separator } from '@/components/ui/separator';
import { api } from '@/trpc/react';
import type { KnowledgeZapQuestionObjects, MatchingVariant, MultipleChoiceVariant, OrderingVariant } from '@/app/(main)/activity/[activityId]/knowledge-zap/live/types';
import MatchingQuestion from '@/app/(main)/activity/[activityId]/knowledge-zap/live/_components/matching-question';
import { motion, AnimatePresence } from 'framer-motion';
import { LightningBoltIcon } from '@/components/icons';
import MultipleChoiceQuestion from '@/app/(main)/activity/[activityId]/knowledge-zap/live/_components/multiple-choice-question';
import OrderingQuestion from '@/app/(main)/activity/[activityId]/knowledge-zap/live/_components/ordering-question';

interface KnowledgeZapPreviewViewProps {
  assignmentAttemptId: string;
  knowledgeZapAssignment?: RouterOutputs["knowledgeZap"]["getKnowledgeZapAssignment"];
...
const renderQuestion = (
  ...
  const randomVariants: (MatchingVariant | MultipleChoiceVariant | OrderingVariant)[] = []
...
const KnowledgeZapPreview: React.FC<KnowledgeZapPreviewViewProps> = ({ 
  assignmentAttemptId,
  ...
  const { data: concepts = [], isLoading: isConceptsLoading } = api.knowledgeZap.getAssignmentConceptsById.useQuery(
    { assignmentId }, 
    ...
    return (
      <div className="flex flex-row items-center gap-1 w-full justify-center mx-auto my-6">
        <AnimatePresence mode="popLayout">
          {questionStack.slice(0, iconsToRender).map((question) => (
            <motion.div
              key={question.id}
      ...
      <div className="w-full border-b border-slate-200 bg-white">
        <div className="px-4 sm:px-8 py-4 sm:py-3">
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-3 sm:hidden">
            ...
            <div className="flex justify-end">
              { role == Roles.Teacher && knowledgeZapAttemptId.length > 0 && (
                <ConceptsModal 
                  topic={topic}
                  classroomId={classroomId}
                  concepts={concepts}
...
export default KnowledgeZapPreview; 