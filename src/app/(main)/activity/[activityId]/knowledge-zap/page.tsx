...
import AssignmentShareModal from "./live/_components/knowledge-zap-assignment-share-modal";
import { getMetaDataFromActivityType } from "@/lib/utils/activityUtils";
import AnalyticsCards from "./_components/analytics-cards";
import { BarChartIcon, FileTextIcon, GridIcon } from "@/components/icons";
import SubmissionsTable from "./_components/submission-table";
import HeatMap from "./_components/heat-map";
...
export default async function AssignmentPage(props: { params: Promise<{ activityId: string }> }) {
  ...
  return (
    <main className="flex flex-col">
      {/* Header */}
      <div className="mb-8 p-4 md:p-8 bg-lime-100">
        <div className="flex flex-col gap-4 mb-4">
          <div className="flex flex-row gap-4">
            <Image src={activityMetaData.iconImage} alt={activityMetaData.title} width={60} height={60} />
            <div className="flex flex-col my-auto">
              <h1 className="text-xl md:text-2xl font-bold text-lime-700">{activityMetaData.title}</h1>
              <p className="text-lime-700">{activity?.topic?.name}</p>
            </div>
          </div>
          <div className="flex flex-row gap-4 md:ml-auto md:mr-4">
      ...
      {/* Analytics */}
      <div className="mb-12 px-4 md:px-8 flex flex-col gap-4">
        <div className="flex flex-row gap-2">
      ...
      {/* Submissions */}
      <div className="mb-8 px-4 md:px-8 flex flex-col gap-4">
        <div className="flex flex-row gap-2">
