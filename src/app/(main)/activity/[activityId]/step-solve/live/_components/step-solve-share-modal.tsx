...
export default function AssignmentShareModal({ 
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    posthog.capture("step_solve_live_published", {
  ...
  useEffect(() => {
    ...
    setAssignmentLink(`${window.location.origin}${Paths.Activity}${activityId}${Paths.StepSolve}${Paths.LiveActivity}`);
  }, [activityId]);

  const copyToClipboard = async () => {
    posthog.capture("step_solve_live_share_copied");
  ...
  return (
    <Dialog>
      <DialogTrigger asChild onClick={() => {
        posthog.capture("step_solve_live_share_clicked");
      }}>
        <Button size="sm" className="bg-teal-700 hover:bg-teal-900">
          Share
          <Share1Icon />
        </Button>
      </DialogTrigger>
